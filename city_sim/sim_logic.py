...
личная жизнь — аренда квартиры, еда, транспорт, расходы на себя и девушку. Это ~{personal_weekly_expenses:.0f} руб за
10-дневный период списывается с твоего личного счёта АВТОМАТИЧЕСКИ и отдельно от решений по бизнесу (не указывай
это в cash_delta — это уже учтено самой симуляцией), но ты должен это учитывать при планировании: деньги утекают
каждый период, даже если бизнес пока не приносит дохода, так что затягивать с выходом на выручку нельзя.
...
и постепенно выйти на ПАССИВНУЮ ежемесячную прибыль от {monthly_profit_goal:.0f} руб — и успеть за {weeks_left}
периодов по 10 дней (дедлайн на периоде {deadline_week}, это ~6 месяцев). Это единственный реальный шанс — действуй
максимально решительно, будь готов пойти на риск (кредиты, агрессивную экспансию, действия на грани дозволенного
или откровенно незаконные, если считаешь риск оправданным), лишь бы приблизиться к цели в срок. Не сдавайся при
первой трудности.
Один твой ход = одно решение за 10-дневный период (не за неделю). Принимай ОДНО конкретное действие (поиск/аренда
помещения, переговоры, обустройство, реклама, оформление документов, работа с клиентами и т.п.), учитывая
макроэкономику и конкуренцию. Нарушения закона могут привлечь внимание гос. органов и обернуться штрафом — это
реальный риск, а не запрет на такие действия.

ВАЖНО ПРО ДЕТАЛИЗАЦИЮ: "action" и "reasoning" должны быть МАКСИМАЛЬНО подробными, а не одной общей фразой. Всегда
указывай конкретику, где она applicable: точные суммы (аренда в руб/мес, цена бокса, сумма депозита, размер кредита
и ставка/срок), имена людей (клиент, арендодатель, инспектор) и их мотив/ситуацию, размеры и количество боксов,
условия договора (срок аренды, что входит, штрафы за просрочку), как именно нашёл клиента или партнёра (через какую
площадку/канал, сколько было звонков/просмотров прежде чем договорились), и как прошли переговоры (кто на чём
настаивал, на чём сошлись). Пиши это как подробный дневниковый отчёт о 10 днях, а не как заголовок новости.
Отвечай СТРОГО одним JSON-объектом, без текста вокруг:
{{"action": "подробное описание действия с конкретными деталями (суммы, имена, условия)", "reasoning": "почему именно это, с деталями рассуждений", "cash_delta": число, "revenue_change": число, "cost_change": число, "risk_flag": true/false, "notes": "что могли бы заметить гос. органы, если risk_flag true"}}"""
...
def businessman_prompt(
    ...
    user = (
        f"{city.describe()}\n\n"
        f"Твоё текущее состояние: капитал {agent.cash:.0f} руб, выручка за период {agent.weekly_revenue:.0f}, "
        f"издержки за период {agent.weekly_costs:.0f}, периодов подряд в плюсе: {agent.consecutive_profitable_weeks}.\n"
        f"Недавняя история:\n{history_text}\n\n"
        f"Прими решение на этот 10-дневный период."
    )
    return system, user
...
def apply_business_result(biz: Agent, result: dict, city_week: int, history_keep: int) -> float:
    ...
    line = (
        f"День {city_week * 10}: {result.get('action', '?')} "
